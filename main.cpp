...
программа, которая выделяет 32КБ памяти, заполняет адресным кодом 1 буфер, обратным адресным кодом - 2 буфер,
записывает в файлы в бинарном виде, считывает с файлов, производит циклический сдвиг на N = 5
...
int main()
{
	...
	*ptr_1 = read_from_file("file_2.bin", *ptr_1);
	...
	write_to_file("file_3.bin", *ptr);
	write_to_file("file_4.bin", *ptr_1);
